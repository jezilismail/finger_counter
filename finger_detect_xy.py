'''
This version only considers x and y coordinates in the landmarks.

The final version in the app.py file also includes the z axis, 
which showed slightly better performance.
'''
...
def get_centroid(pnt_list):
    ...
    centroid = [x / MCP_len, y / MCP_len]
    # print(x, y, centroid) # see x, y vals and centroid 
    return centroid
    ...
    if result.multi_hand_landmarks:
        for hand_lmrks in result.multi_hand_landmarks:
            ...
            for id, lm in enumerate(hand_lmrks.landmark):
                ...
                point_x = lm.x
                point_y = lm.y
                lm_list.append([id, point_x, point_y])
